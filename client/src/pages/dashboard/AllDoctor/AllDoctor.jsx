...
const { data: doctorDetails = [] } = useQuery({
  ...
  const handleOpen = (doctor) => {
    ...
    return (
          ...
          <tbody >
            {doctorDetails?.map((doctor, index) => <tr className="font-barlow" key={index}>
              <td className={classes}>
                ...
                >
                  <span className="font-semibold">Name: </span>{doctor?.name}<br />
                  <span className="font-semibold">Email: </span>{doctor?.email}<br />
                  <span className="font-semibold">Blood Group: </span>{doctor?.blood_group}
                </Typography>
              </td>
              <td className={classes}>
                <Typography
              ...
              <td className={classes}>
                ...
                >
                  <span className="font-semibold">Location: </span>{doctor?.loc}<br />
                  <span className="font-semibold">Category: </span>{doctor?.health_category}<br />
                  <span className="font-semibold">Registration No: </span>{doctor?.registration}<br />

                </Typography>
              </td>
              <td className={classes}>
                <Button color="blue" onClick={() => handleOpen(doctor)} className="px-2 py-1 rounded-none mr-2 normal-case">Manage</Button>
      ...
      {/* <ManageModal handleOpen={handleOpen} open={open} doctor={doctorData}></ManageModal> */}
    </div >
    );
};
